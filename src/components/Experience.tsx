...
export default function Experience() {
  ...
  return (
    <section id="experience" className="py-2 md:py-5 bg-slate-50 dark:bg-slate-900 relative">
      <div className="max-w-7xl mx-auto w-full px-6 md:px-12">
        {/* Section Header */}
        <div className="mb-4 md:mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b-0 md:border-b border-slate-200/60 dark:border-slate-800/60">
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, margin: "-100px" }}
            transition={{ duration: 0.5 }}
            className="hidden md:flex text-left md:w-auto mb-2 md:mb-0"
          >
            <h2 className="text-xs sm:text-sm font-extrabold uppercase tracking-wider text-slate-950 dark:text-slate-50">
              Pengalaman
            </h2>
          </motion.div>
 
          <div 
            className="flex flex-row w-full md:w-auto overflow-x-auto [&::-webkit-scrollbar]:hidden"
            ...
            {tabs.map(tab => {
              ...
              return (
                <button
                  ...
                  className={`flex-1 md:flex-none text-center px-5 md:px-6 py-3 text-xs sm:text-sm font-bold transition-all duration-200 relative cursor-pointer whitespace-nowrap ${
                    isActive 
                      ? 'text-[#02227E] dark:text-blue-400' 
                ...
                >
                  ...
                  {isActive && (
                    <motion.div 
                      layoutId="activeTabUnderlineExperience"
                      className="absolute bottom-0 left-0 right-0 h-[2px] bg-[#02227E] dark:bg-blue-400"
                    />
                  )}
                </button>
              );
