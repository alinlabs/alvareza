...
export default function Dashboard({ setActiveTab }: DashboardProps) {
  ...
  useEffect(() => {
    ...
    // Load recruiters count
    ApiService.get<any>('rekruter').then(res => {
      if (res.success && res.data && Array.isArray(res.data)) {
        setTotalRecruiters(res.data.length);
      }
    });
  }, []);
  ...
  return (
    <div id="admin-dashboard-page" className="space-y-8 text-left">
      ...
      {/* Grid Stats: 4 Cards (Total Cover Letter | Total Draft | Total Apply | Total Rekruter) */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
        {/* Card 1: Total Cover Letter */}
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-3.5 sm:p-5 flex items-center justify-between shadow-sm dark:shadow-none min-w-0">
          <div className="flex items-center gap-2.5 sm:gap-4 min-w-0">
            <div className="p-2.5 sm:p-3 bg-slate-100 dark:bg-slate-950 text-accent dark:text-accent rounded-xl border border-slate-200 dark:border-slate-850 shrink-0">
              <FileText className="w-4.5 h-4.5 sm:w-5.5 sm:h-5.5" />
            </div>
            <div className="min-w-0">
              <p className="text-xl sm:text-2xl font-extrabold text-slate-900 dark:text-slate-100 leading-none">{totalTemplates}</p>
              <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mt-1 truncate">Total Cover Letter</p>
            </div>
          </div>
          <button 
            onClick={() => setActiveTab('email-sender')}
            className="p-1 sm:p-1.5 bg-slate-100 dark:bg-slate-950 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-accent dark:hover:text-accent rounded-lg border border-slate-200 dark:border-slate-850 cursor-pointer transition-colors shrink-0 ml-1"
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-3.5 sm:p-5 flex items-center justify-between shadow-sm dark:shadow-none min-w-0">
          <div className="flex items-center gap-2.5 sm:gap-4 min-w-0">
            <div className="p-2.5 sm:p-3 bg-slate-100 dark:bg-slate-950 text-amber-600 dark:text-amber-400 rounded-xl border border-slate-200 dark:border-slate-850 shrink-0">
              <Briefcase className="w-4.5 h-4.5 sm:w-5.5 sm:h-5.5" />
            </div>
            <div className="min-w-0">
              <p className="text-xl sm:text-2xl font-extrabold text-amber-600 dark:text-amber-400 leading-none">{totalDrafts}</p>
              <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mt-1 truncate">Total Draft</p>
            </div>
          </div>
          <button 
            onClick={() => setActiveTab('email-sender')}
            className="p-1 sm:p-1.5 bg-slate-100 dark:bg-slate-950 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-amber-600 dark:hover:text-amber-400 rounded-lg border border-slate-200 dark:border-slate-850 cursor-pointer transition-colors shrink-0 ml-1"
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-3.5 sm:p-5 flex items-center justify-between shadow-sm dark:shadow-none min-w-0">
          <div className="flex items-center gap-2.5 sm:gap-4 min-w-0">
            <div className="p-2.5 sm:p-3 bg-slate-100 dark:bg-slate-950 text-emerald-600 dark:text-emerald-400 rounded-xl border border-slate-200 dark:border-slate-850 shrink-0">
              <Send className="w-4.5 h-4.5 sm:w-5.5 sm:h-5.5" />
            </div>
            <div className="min-w-0">
              <p className="text-xl sm:text-2xl font-extrabold text-emerald-600 dark:text-emerald-400 leading-none">{totalApplied}</p>
              <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mt-1 truncate">Total Apply</p>
            </div>
          </div>
          <button 
            onClick={() => setActiveTab('email-sender')}
            className="p-1 sm:p-1.5 bg-slate-100 dark:bg-slate-950 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 rounded-lg border border-slate-200 dark:border-slate-850 cursor-pointer transition-colors shrink-0 ml-1"
        ...
        {/* Card 4: Total Rekruter */}
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-3.5 sm:p-5 flex items-center justify-between shadow-sm dark:shadow-none min-w-0">
          <div className="flex items-center gap-2.5 sm:gap-4 min-w-0">
            <div className="p-2.5 sm:p-3 bg-slate-100 dark:bg-slate-950 text-indigo-600 dark:text-indigo-400 rounded-xl border border-slate-200 dark:border-slate-850 shrink-0">
              <Users className="w-4.5 h-4.5 sm:w-5.5 sm:h-5.5" />
            </div>
            <div className="min-w-0">
              <p className="text-xl sm:text-2xl font-extrabold text-indigo-600 dark:text-indigo-400 leading-none">{totalRecruiters}</p>
              <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mt-1 truncate">Total Rekruter</p>
            </div>
          </div>
          <button 
            onClick={() => setActiveTab('rekruter')}
            className="p-1 sm:p-1.5 bg-slate-100 dark:bg-slate-950 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 rounded-lg border border-slate-200 dark:border-slate-850 cursor-pointer transition-colors shrink-0 ml-1"
            title="Lihat data rekruter"
          >
            <ArrowUpRight className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
          </button>
        </div>
