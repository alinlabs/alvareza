      ...
      try {
        ...
        const emailBodyHtml = `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px;">
            <h2 style="color: #2c3e50; border-bottom: 2px solid #3498db; padding-bottom: 10px;">Kode OTP Verifikasi Admin</h2>
            <p style="font-size: 16px; color: #333; line-height: 1.5;">Halo ${profile.nama || 'Admin'},</p>
            <p style="font-size: 16px; color: #333; line-height: 1.5;">Seseorang mencoba masuk ke Dashboard Admin menggunakan fitur Lupa Kata Sandi.</p>
            <div style="background-color: #f8f9fa; border: 1px dashed #3498db; padding: 15px; border-radius: 6px; text-align: center; margin: 20px 0;">
              <span style="font-size: 28px; font-weight: bold; letter-spacing: 4px; color: #2c3e50;">${generatedCode}</span>
            </div>
            <p style="font-size: 14px; color: #666; line-height: 1.5;">Kode ini hanya berlaku untuk sesi login saat ini. Harap jangan memberikan kode ini kepada siapapun demi menjaga keamanan akun Anda.</p>
            <hr style="border: 0; border-top: 1px solid #e0e0e0; margin: 20px 0;" />
            <p style="font-size: 12px; color: #999; text-align: center;">Sistem Portofolio Admin &bull; Alvareza Hilka Pratama</p>
          </div>
        `;

        if (gasWebAppUrl && gasWebAppUrl.startsWith('https://script.google.com/')) {
          const payload = {
            targetEmail: correctEmail,
            cc: '',
            bcc: '',
          ...
          if (response.ok) {
            setInfoMessage(`Kode OTP berhasil dikirim ke email Gmail Anda (${correctEmail}) melalui Google Apps Script.`);
            setSimulatedOtpNotice('');
          } else {
            setSimulatedOtpNotice(generatedCode);
            setInfoMessage('Gagal mengirim OTP via Google Apps Script. Mengaktifkan kode OTP simulasi lokal.');
          }
        } else {
          setSimulatedOtpNotice(generatedCode);
          setInfoMessage('Google Apps Script Web App URL belum dikonfigurasi. Mengaktifkan kode OTP simulasi lokal.');
        }
      } catch (err) {
