...
// Helper function to load image dimensions and base64 for jsPDF with quality parameter
export const loadImageData = (url: string, qualityPercent: number = 80): Promise<{ dataUrl: string, width: number, height: number }> => {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      const maxDim = 2200; // Optimal max dimension for crisp text (>200 DPI on A4) while keeping PDF lightweight
...
export function useBerkas() {
  const [activeTab, setActiveTab] = useState<'list' | 'merge' | 'img2pdf'>(() => {
    const path = window.location.pathname;
    if (path.startsWith('/admin/berkas/')) {
      const sub = path.split('/')[3];
      if (['list', 'merge', 'img2pdf'].includes(sub)) {
        return sub as any;
      }
    }
    return 'list';
  ...
  useEffect(() => {
    // Only update if we are currently in berkas
    if (window.location.pathname.startsWith('/admin/berkas')) {
      const newPath = `/admin/berkas/${activeTab}`;
  ...
  const [mergeCompressionPercent, setMergeCompressionPercent] = useState<number>(85);
  ...
  const [img2pdfCompressionPercent, setImg2pdfCompressionPercent] = useState<number>(80);
  ...
  const handleMergePDFs = async () => {
    ...
    try {
      ...
      if (mergeCompressionPercent < 100 || optimizePdf) {
        const qualityLevel = mergeCompressionPercent < 100 ? mergeCompressionPercent : 85;
        setMergeProgress(`Mengoptimalkan & mengompresi PDF (${qualityLevel}% kualitas)...`);
        mergedPdfBytes = new Uint8Array(await compressPdf(mergedPdfBytes, setMergeProgress, qualityLevel));
      }
...
// Helper function to generate tiny compressed thumbnails for fast UI rendering without memory lag
const createThumbnail = (file: File): Promise<string> => {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      try {
        const canvas = document.createElement('canvas');
        const maxDim = 120; // 120px is perfect for the 40x40 preview box
        let width = img.width;
        ...
        // Fallback to objectUrl on error
      }
      resolve(objectUrl);
    };
  ...
  const addLocalImages = async (fileList: FileList) => {
    const rawFiles = Array.from(fileList).filter(file => file.type.startsWith('image/') || file.name.match(/\.(jpeg|jpg|gif|png|webp|bmp)$/i) != null);
    if (rawFiles.length === 0) {
      showToast('Hanya mendukung berkas tipe gambar (PNG, JPG, WEBP, dll)');
      return;
    }
    ...
    const newItems: QueueItem[] = await Promise.all(
      rawFiles.map(async (file, index) => {
        ...
        return {
          id: `local-img-${timestamp}-${index}-${Math.random().toString(36).substring(2, 6)}`,
          name: file.name,
          url: '',
          file: file,
          type: 'local',
  ...
  const handleImagesToPdf = async () => {
    ...
    try {
      ...
      for (let i = 0; i < imageQueue.length; i++) {
        ...
        const imgInfo = await loadImageData(imgUrl, img2pdfCompressionPercent);
