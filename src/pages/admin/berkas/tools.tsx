...
interface BerkasMergeProps {
  ...
  mergeCompressionPercent: number;
  setMergeCompressionPercent: (val: number) => void;
...
export function BerkasMerge({
  ...
  optimizePdf,
  setOptimizePdf,
  mergeCompressionPercent,
  setMergeCompressionPercent,
  pdfSearchQuery,
  setPdfSearchQuery,
  merging,
  mergeProgress,
        ...
        >
          ...
          <div className="border-t border-slate-100 dark:border-slate-800 pt-4 space-y-4 shrink-0">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2.5 bg-slate-50 dark:bg-slate-900/60 p-3 rounded-xl border border-slate-200 dark:border-slate-800 text-left">
                <div className="flex items-center justify-between">
                  <label className="text-[11px] font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                    <span>Kompresi & Kualitas PDF Hasil</span>
                  </label>
                  <span className="text-[10px] font-extrabold text-accent bg-accent/10 px-2 py-0.5 rounded-md">
                    {mergeCompressionPercent === 100 ? '100% (Asli)' : `${mergeCompressionPercent}% Kualitas`}
                  </span>
                </div>

                <div className="grid grid-cols-4 gap-1.5">
                  {[
                    { label: '100%', sub: 'Asli', value: 100 },
                    { label: '85%', sub: 'Optimal', value: 85 },
                    { label: '70%', sub: 'Sedang', value: 70 },
                    { label: '50%', sub: 'Ringan', value: 50 },
                  ].map(opt => (
                    <button
                      type="button"
                      key={opt.value}
                      onClick={() => {
                        setMergeCompressionPercent(opt.value);
                        setOptimizePdf(opt.value < 100);
                      }}
                      className={`py-1.5 px-1 rounded-lg text-center cursor-pointer transition-all border ${
                        mergeCompressionPercent === opt.value
                    ...
                    >
                      ...
                      <div className="text-[9px] opacity-80 font-medium">{opt.sub}</div>
                    </button>
                  ))}
                </div>
                ...
                <div className="flex items-center gap-2 pt-0.5">
                  <input
                    ...
                    max="100"
                    step="5"
                    value={mergeCompressionPercent}
                    onChange={(e) => {
                      const val = Number(e.target.value);
                      setMergeCompressionPercent(val);
                      setOptimizePdf(val < 100);
                    }}
                    className="w-full accent-accent h-1.5 bg-slate-200 dark:bg-slate-700 rounded-lg cursor-pointer"
                  />
                </div>

                <p className="text-[10px] text-slate-500 dark:text-slate-400 leading-tight">
                  {mergeCompressionPercent === 100
                    ? '✓ Format vektor asli dipertahankan tanpa perubahan resolusi.'
                    : mergeCompressionPercent >= 85
                    ? '⚡ Rekomendasi: Teks tetap tajam & jernih dengan ukuran file lebih ringan.'
                    : mergeCompressionPercent >= 70
                    ? '📦 Kompresi sedang untuk menghemat ukuran penyimpanan.'
                    : '⚠️ Kompresi tinggi untuk hasil file sangat kecil & ringan.'}
                </p>
              </div>
            </div>
...
interface BerkasImg2PdfProps {
  ...
  img2pdfCompressionPercent: number;
  setImg2pdfCompressionPercent: (val: number) => void;
...
export function BerkasImg2Pdf({
  ...
  img2pdfMargin,
  setImg2pdfMargin,
  img2pdfCompressionPercent,
  setImg2pdfCompressionPercent,
  imageSearchQuery,
  setImageSearchQuery,
  converting,
  convertProgress,
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto space-y-2 pr-1 my-2 lg:min-h-0 flex flex-col">
            {imageQueue.length > 0 && imageQueue.map((item, index) => (
              <div 
                key={item.id}
                className="flex items-center justify-between p-2.5 bg-slate-50 dark:bg-slate-950 rounded-xl shrink-0 border border-slate-100 dark:border-slate-800/60 hover:border-slate-200 dark:hover:border-slate-800 transition-colors"
              >
                <div className="flex items-center gap-3 min-w-0 pr-3">
                  <span className="text-xs font-bold text-slate-400 dark:text-slate-600 bg-white dark:bg-slate-900 w-5 h-5 rounded-full border border-slate-200 dark:border-slate-800 flex items-center justify-center shrink-0">
                    {index + 1}
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <button 
                    ...
                    className="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-900 text-slate-400 hover:text-slate-700 dark:hover:text-slate-300 disabled:opacity-30 disabled:pointer-events-none cursor-pointer"
                  ...
                  <button 
                    ...
                    className="p-1 rounded hover:bg-rose-50 dark:hover:bg-rose-950/30 text-slate-400 hover:text-rose-500 cursor-pointer"
          ...
          <div className="border-t border-slate-100 dark:border-slate-800 pt-4 space-y-4 shrink-0">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
              ...
              <div className="space-y-1 text-left">
                ...
                <select
                  value={img2pdfCompressionPercent}
                  onChange={e => setImg2pdfCompressionPercent(Number(e.target.value))}
                  className="w-full px-2 py-1.5 text-[11px] bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl focus:border-accent focus:outline-none cursor-pointer font-bold text-accent"
                >
                  <option value={100}>100% - Kualitas Asli</option>
                  <option value={85}>85% - Sangat Tajam</option>
                  <option value={80}>80% - Ringan & Terbaca (Default)</option>
                  <option value={70}>70% - Sedang</option>
                  <option value={50}>50% - Ringan Sekali</option>
                </select>
              </div>
            </div>

            {/* Percentage Slider & Quick Select Control */}
            <div className="bg-slate-50 dark:bg-slate-900/60 p-2.5 rounded-xl border border-slate-200 dark:border-slate-800 space-y-2 text-left">
              <div className="flex items-center justify-between text-[10px] font-bold text-slate-600 dark:text-slate-400">
                <span className="flex items-center gap-1.5">
                  Tingkat Kompresi & Kualitas Gambar
                  {img2pdfCompressionPercent === 80 && (
                    <span className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[9px] px-1.5 py-0.2 rounded font-extrabold">Default Ringan & Terbaca</span>
                  )}
                </span>
                <span className="bg-accent/10 text-accent px-2 py-0.5 rounded font-extrabold">{img2pdfCompressionPercent}%</span>
              </div>

              <div className="grid grid-cols-4 gap-1.5">
                {[
                  { label: '100%', sub: 'Asli', value: 100 },
                  { label: '80%', sub: 'Ringan (Default)', value: 80 },
                  { label: '70%', sub: 'Sedang', value: 70 },
                  { label: '50%', sub: 'Ringan Sekali', value: 50 },
                ].map(opt => (
                  <button
                    type="button"
                    key={opt.value}
                    onClick={() => setImg2pdfCompressionPercent(opt.value)}
                    className={`py-1.5 px-1 rounded-lg text-center cursor-pointer transition-all border ${
                      img2pdfCompressionPercent === opt.value
                        ? 'bg-accent text-white border-accent shadow-xs font-bold'
                        : 'bg-white dark:bg-slate-950 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-800 hover:border-slate-300'
                    }`}
                  >
              ...
              <div className="flex justify-between text-[9px] text-slate-400 font-medium">
                <span>30% Ringan Sekali</span>
                <span>80% Ringan & Terbaca (Optimal)</span>
                <span>100% Asli</span>
              </div>
            </div>
