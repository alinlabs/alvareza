...
export default function KeahlianEditor() {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const response = await ApiService.get<any>('keahlian');
      if (!response.success) {
        throw new Error(response.message || 'Gagal memuat data keahlian');
      }
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await ApiService.post('keahlian', dataObj);
      if (!response.success) {
        throw new Error(response.error || response.message || 'Gagal menyimpan data');
      }
      
      setSuccessMsg('Data keahlian berhasil disimpan!');
      setTimeout(() => {
        setSuccessMsg(null);
      }, 3000);
    } catch (err: any) {
  ...
  const handleDeleteItem = async (index: number) => {
    ...
    if (item.id && window.confirm('Apakah Anda yakin ingin menghapus data ini?')) {
      try {
        const response = await ApiService.delete('keahlian', { body: JSON.stringify({ id: item.id }) });
        if (!response.success) {
    ...
    return (
      <div className="flex items-center justify-center min-h-[50vh]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-slate-500 text-sm">Memuat data keahlian...</p>
        </div>
      </div>
    );
  }
        ...
        {dataObj.map((item, index) => (
          <div key={index} className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 md:p-10 shadow-sm animate-fadeIn">
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800/60 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-bold uppercase tracking-wider text-[#1e293b] dark:text-[#f8fafc] line-clamp-1">
                    {item.bidang ? item.bidang : 'Keahlian Baru'}
                  </h3>
                </div>
                <button
                  type="button"
              ...
              {/* Row 1: Kategori | Bidang | Persentase */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="space-y-1.5">
                    ...
                    <input 
                      ...
                      placeholder="Komunikasi, Teknologi"
                    />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input 
                      ...
                      placeholder="Negosiasi, Web Development"
                    />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input 
                      type="number" 
                      className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2.5 text-xs sm:text-sm text-[#1e293b] dark:text-[#f8fafc] font-normal focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent transition-all"
                      value={item.persentase !== undefined ? item.persentase : 0} 
                      onChange={(e) => {
                        const newArr = [...dataObj];
                        newArr[index] = { ...item, persentase: parseInt(e.target.value) || 0 };
                        setDataObj(newArr);
                      }} 
                      placeholder="Persentase penguasaan (0-100)"
                      min="0"
                      max="100"
                    />
                  </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <AutoResizingTextarea 
                    ...
                    placeholder="Tulis deskripsi singkat mengenai keahlian ini..."
                  />
                </div>

                {/* Row 3: Penguasaan (Sub-Keahlian) */}
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button 
                      onClick={() => {
                        const newArr = [...dataObj];
                        const currentSub = Array.isArray(item.penguasaan) ? item.penguasaan : [];
                        newArr[index] = { ...item, penguasaan: [...currentSub, ''] };
                        setDataObj(newArr);
                      }}
                      className="px-4 py-2 text-xs font-bold bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors cursor-pointer border border-slate-200 dark:border-slate-700 inline-flex items-center gap-1.5"
                    >
                      <Plus className="w-3.5 h-3.5" /> Tambah Penguasaan
                    </button>
                  </div>
                  
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3.5">
                    {(Array.isArray(item.penguasaan) ? item.penguasaan : []).map((sub, sIdx) => (
                      <div key={sIdx} className="flex items-center gap-2">
                        <input 
                          type="text" 
                          className="flex-grow bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2 text-xs sm:text-sm text-[#1e293b] dark:text-[#f8fafc] font-normal focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent"
                          value={sub} 
                          onChange={(e) => {
                            const newArr = [...dataObj];
                            const newSub = [...(Array.isArray(item.penguasaan) ? item.penguasaan : [])];
                            newSub[sIdx] = e.target.value;
                            newArr[index] = { ...item, penguasaan: newSub };
                            setDataObj(newArr);
                          }} 
                          placeholder="Sub Keahlian"
                        />
                        <button 
                          onClick={() => {
                            const newArr = [...dataObj];
                          ...
                          className="text-red-500 hover:text-red-700 p-2 hover:bg-red-50 dark:hover:bg-red-950/30 rounded-xl transition-colors cursor-pointer shrink-0"
                          title="Hapus Sub Keahlian"
                        >
