...
export default function PencapaianEditor() {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const response = await ApiService.get<any>('pencapaian');
      if (!response.success) {
        throw new Error(response.message || 'Gagal memuat data pencapaian');
      }
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await ApiService.post('pencapaian', dataObj);
      if (!response.success) {
        throw new Error(response.error || response.message || 'Gagal menyimpan data');
      }
      
      setSuccessMsg('Data pencapaian berhasil disimpan!');
      setTimeout(() => {
        setSuccessMsg(null);
      }, 3000);
    } catch (err: any) {
  ...
  const handleDeleteItem = async (index: number) => {
    ...
    if (item.id && window.confirm('Apakah Anda yakin ingin menghapus data ini?')) {
      try {
        const response = await ApiService.delete('pencapaian', { body: JSON.stringify({ id: item.id }) });
        if (!response.success) {
    ...
    return (
      <div className="flex items-center justify-center min-h-[50vh]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-slate-500 text-sm">Memuat data pencapaian...</p>
        </div>
      </div>
    );
  }
        ...
        {dataObj.map((item, index) => (
          <div key={index} className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 md:p-10 shadow-sm animate-fadeIn">
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800/60 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-bold uppercase tracking-wider text-[#1e293b] dark:text-[#f8fafc] line-clamp-1">
                    {item.judul ? item.judul : 'Pencapaian Baru'}
                  </h3>
                </div>
                <button
                  type="button"
              ...
              {/* Row 1: Judul | Penerbit/Penyelenggara | Kategori */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="space-y-1.5 md:col-span-2">
                  ...
                  <input 
                    ...
                    placeholder="Presentator Meeting Nasional Konica Minolta"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <input 
                    ...
                    placeholder="Kepanitiaan, Moderator, Presentasi"
                  />
                </div>
              </div>

              {/* Row 2: Tingkat | Penerbit | Tahun */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="space-y-1.5">
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400">
                    Tingkat
                  ...
                  <input 
                    ...
                    onChange={(e) => {
                      const newArr = [...dataObj];
                      newArr[index] = { ...item, tingkat: e.target.value };
                      setDataObj(newArr);
                    }} 
                    placeholder="Tingkat Nasional, Tingkat Provinsi"
                  />
                </div>
              ...
              <div className="space-y-1.5">
                ...
                <AutoResizingTextarea 
                  ...
                  placeholder="Tulis deskripsi ringkas mengenai jalannya pencapaian ini..."
                />
              </div>

              {/* Row 4: Sertifikat URL */}
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  placeholder="/gambar/sertifikat/1.webp"
                />
