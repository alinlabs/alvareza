...
export default function ProfilEditor() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const localGasUrl = localStorage.getItem('gasWebAppUrl');
      if (localGasUrl) {
        data.gasWebAppUrl = localGasUrl;
      }
      setDataObj(data);
    } catch (err: any) {
  ...
  const handleSave = async () => {
    ...
    try {
      // Remove gasWebAppUrl from payload so it doesn't fail on Cloudflare D1
      const payload = { ...dataObj };
      if (payload.gasWebAppUrl !== undefined) {
        localStorage.setItem('gasWebAppUrl', payload.gasWebAppUrl);
        delete payload.gasWebAppUrl;
      }
      if (payload.gas_web_app_url !== undefined) {
        delete payload.gas_web_app_url;
      }
      
      const response = await ApiService.post('profil', payload);
      if (!response.success) {
  ...
  const getVal = (key: string) => {
    if (!dataObj) return '';
    return dataObj[key] != null ? dataObj[key] : '';
  };
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 md:p-10 shadow-sm space-y-8 animate-fadeIn">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-1.5">
              ...
              <input 
                type="text" 
                className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2.5 text-xs sm:text-sm text-[#1e293b] dark:text-[#f8fafc] font-normal focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent transition-all"
                value={dataObj?.tempat_lahir != null ? dataObj.tempat_lahir : getVal('tempatLahir')} 
                onChange={(e) => {
                  setDataObj({
                    ...dataObj,
            ...
            <div className="space-y-1.5">
              ...
              <input 
                type="text" 
                className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2.5 text-xs sm:text-sm text-[#1e293b] dark:text-[#f8fafc] font-normal focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent transition-all"
                value={dataObj?.tanggal_lahir != null ? dataObj.tanggal_lahir : getVal('tanggalLahir')} 
                onChange={(e) => {
                  setDataObj({
                    ...dataObj,
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-1.5">
              <label className="block text-xs font-bold text-slate-500 dark:text-slate-400">
                Google Apps Script (GAS) Web App URL <span className="text-emerald-500 font-extrabold">(Utama / Wajib)</span>
              </label>
              <input 
                type="text" 
                className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2.5 text-xs sm:text-sm text-[#1e293b] dark:text-[#f8fafc] font-normal focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent transition-all"
                value={dataObj?.gasWebAppUrl != null ? dataObj.gasWebAppUrl : getVal('gasWebAppUrl')} 
                onChange={(e) => {
                  setDataObj({
                    ...dataObj,
                    gasWebAppUrl: e.target.value
                  });
                }} 
                placeholder="https://script.google.com/macros/s/.../exec"
              />
              <p className="text-[10px] text-slate-400 font-medium leading-relaxed">
                Satu-satunya sistem pengiriman email aktif. Sangat andal, mem-bypass limit proxy, dan mampu melampirkan file hingga 50MB langsung ke server Google.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-1.5">
              ...
              <input 
                type="text" 
                className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2.5 text-xs sm:text-sm text-[#1e293b] dark:text-[#f8fafc] font-normal focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent transition-all"
                value={dataObj?.pendidikan_terakhir != null ? dataObj.pendidikan_terakhir : getVal('pendidikanTerakhir')} 
                onChange={(e) => {
                  setDataObj({
                    ...dataObj,
