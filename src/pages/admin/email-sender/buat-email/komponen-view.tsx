...
export const CustomSelect = ({ 
  ...
  options: { value: string; label: React.ReactNode; searchValue?: string; selectedLabel?: React.ReactNode; isHeader?: boolean }[];
  placeholder?: string;
  searchable?: boolean;
}) => {
  ...
  // Filter options including headers
  const getFilteredOptions = () => {
    if (!searchTerm) return options;
    
    const result: typeof options = [];
    let currentHeader: typeof options[0] | null = null;
    let hasMatchingItemsInGroup = false;

    for (const opt of options) {
      if (opt.isHeader) {
        currentHeader = opt;
        hasMatchingItemsInGroup = false;
      } else {
        const textToSearch = opt.searchValue || (typeof opt.label === 'string' ? opt.label : opt.value);
        const isMatch = textToSearch.toLowerCase().includes(searchTerm.toLowerCase());
        
        if (isMatch) {
          if (currentHeader && !hasMatchingItemsInGroup) {
            result.push(currentHeader);
            hasMatchingItemsInGroup = true;
          }
          result.push(opt);
        }
      }
            ...
            filteredOptions.map((opt) => {
              if (opt.isHeader) {
                return (
                  <div
                    key={opt.value}
                    className="px-4 py-2 bg-slate-100 dark:bg-[#1e293b] text-[10px] font-bold text-slate-600 dark:text-slate-400 tracking-wider uppercase select-none border-y border-slate-200/60 dark:border-slate-800/80 cursor-default"
                  >
                    {opt.label}
                  </div>
                );
...
export const formatFileSize = (file: File | Blob | number | null | undefined, defaultLabel = "Bawaan (System)") => {
  if (file === null || file === undefined) return defaultLabel;
  let bytes = 0;
  if (typeof file === 'number') {
    bytes = file;
  } else if (file && typeof (file as any).size === 'number') {
    bytes = (file as any).size;
  } else {
    return defaultLabel;
  }
  if (bytes <= 0) return defaultLabel;
