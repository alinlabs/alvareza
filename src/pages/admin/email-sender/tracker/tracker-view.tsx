...
import { JobApplication } from './pratinjau-surat-view';
import { TrackerStats } from './statistik-tracker-view';
import { TrackerItem } from './item-tracker-view';
...
export default function Tracker({ 
  ...
  const handleSendSingleDraft = async (job: JobApplication) => {
    ...
    try {
      ...
      // Fetch compiled HTML from preview endpoint
      ...
      const compiledHtml = previewRes.ok && previewData.html ? previewData.html : job.body;

      const attachmentsList = parseAttachments(job.attachedFiles);
      
      const docMap: Record<string, { url: string; name: string }> = {
        'CV': { url: '/gambar/cv/cv-blue-2026.webp', name: 'CV.pdf' },
        'Curriculum Vitae': { url: '/gambar/cv/cv-blue-2026.webp', name: 'CV.pdf' },
        'Paklaring': { url: '/gambar/paklaring/gmg.webp', name: 'pengalaman.pdf' },
        'Sertifikat': { url: '/gambar/sertifikat/akademis1.webp', name: 'sertifikat.pdf' },
        'Ijazah': { url: '/gambar/sertifikat/akademis1.webp', name: 'sertifikat.pdf' }
      };
      ...
      const activePdfs: { blob: Blob; filename: string }[] = [];
      if (job.mergeAttachments && gatheredAttachments.length > 0) {
        try {
          ...
          activePdfs.push({ blob: mergedBlob, filename: 'berkas_alvareza.pdf' });
        } catch (e: any) {
          if (e.name === 'AbortError') throw e;
          console.error("Gagal menggabungkan PDF:", e);
          for (const item of gatheredAttachments) {
            activePdfs.push({ blob: item.blob, filename: item.filename });
          }
        }
      } else {
        for (const item of gatheredAttachments) {
      ...
      const payload = {
        targetEmail: getJobEmail(job),
        cc: '',
        bcc: '',
        subject: job.subject || `Lamaran Pekerjaan - ${job.positionName} - Alvareza Hilka Pratama`,
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(`Gagal mengirim email via GAS: ${textData.substring(0, 150)}`);
        } else {
      ...
      if (!response.ok || resData.success === false) {
        throw new Error(resData.error || 'Gagal mengirim email via GAS');
      }
    ...
    <div id="admin-tracker-page" className="space-y-8 text-left">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-stretch md:items-center justify-between">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 shrink-0">
          ...
          <div className="flex items-center gap-2 p-1 bg-slate-100 dark:bg-slate-800 rounded-xl w-full sm:w-auto border border-slate-200/50 dark:border-slate-700/50 shrink-0 no-scrollbar">
            <button
              type="button"
