...
export function buildEmailContent(params: GenerateEmailParams): GeneratedEmail {
  ...
  // Replace variables
  const isGeneral = isPositionGeneral || positionName.toLowerCase().trim() === 'umum';
  const repPos = isGeneral ? 'Umum' : (positionName || '[Posisi]');
  ...
  const autoSub = isGeneral
    ? (isEnglish ? `Job Application - Alvareza Hilka Pratama` : `Lamaran Pekerjaan - Alvareza Hilka Pratama`)
    : (isEnglish ? `Job Application: ${repPos} - Alvareza Hilka Pratama` : `Lamaran Pekerjaan: ${repPos} - Alvareza Hilka Pratama`);
...
${tempat}`;
  ...
  if (includePerihal || includeLampiranAwal) {
    ...
    if (includePerihal) {
      ...
      const perihalVal = isGeneral
        ? (isEnglish ? 'Job Application' : 'Lamaran Pekerjaan')
        : (isEnglish ? `Job Application - ${repPos}` : `Lamaran Pekerjaan - ${repPos}`);
